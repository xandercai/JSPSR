...
from models.components.resnet_cbam import BasicBlock
from models.components.pvt import PVT
...
class Backbone(nn.Module):
    def __init__(self, args, mode="rgbd"):
        ...
        in_channels = 3
        if "mask" in self.args.input_channels:
            in_channels += 15
        if "canopy" in self.args.input_channels:
            in_channels += 1

        # Encoder
        if mode == "rgbd":
            self.conv1_rgb = conv_bn_relu(
                in_channels, in_channels * 16, kernel=3, stride=1, padding=1, bn=False
            )
            self.conv1_dep = conv_bn_relu(
                1, 16, kernel=3, stride=1, padding=1, bn=False
            )
            self.conv1 = conv_bn_relu(
                (in_channels + 1) * 16, 64, kernel=3, stride=1, padding=1, bn=False
            )
        elif mode == "rgb":
            self.conv1 = conv_bn_relu(3, 64, kernel=3, stride=1, padding=1, bn=False)
        elif mode == "d":
        ...
        pvt_weight = "./models/pretrained/PVT.pth"
        if os.path.exists(pvt_weight):
            state_dict = torch.load(pvt_weight)
            self.former.load_state_dict(state_dict, strict=False)
            print("PVT weights loaded from {}".format(pvt_weight))
        else:
            print("PVT weights not found in {}".format(pvt_weight))
        ...
        self.dec6 = nn.Sequential(
            convt_bn_relu(
                channels[5], 256, kernel=3, stride=2, padding=1, output_padding=1
            ),
        ...
        self.dec5 = nn.Sequential(
            convt_bn_relu(
                256 + channels[4], 128, kernel=3, stride=2, padding=1, output_padding=1
            ),
        ...
        self.dec4 = nn.Sequential(
            convt_bn_relu(
                128 + channels[3], 64, kernel=3, stride=2, padding=1, output_padding=1
            ),
        ...
        self.dec3 = nn.Sequential(
            convt_bn_relu(
                64 + channels[2], 64, kernel=3, stride=2, padding=1, output_padding=1
            ),
            BasicBlock(64, 64, stride=1, downsample=None, ratio=4),
        )
        ...
        self.dec2 = nn.Sequential(
            convt_bn_relu(
                64 + channels[1], 64, kernel=3, stride=2, padding=1, output_padding=1
            ),
            BasicBlock(64, 64, stride=1, downsample=None, ratio=4),
        )
        ...
        # Init Depth Branch
        # 1/1
        self.dep_dec1 = conv_bn_relu(64 + 64, 64, kernel=3, stride=1, padding=1)
        self.dep_dec0 = conv_bn_relu(
            64 + 64, 1, kernel=3, stride=1, padding=1, bn=False, relu=True
        )
        # Guidance Branch
        # 1/1
        self.gd_dec1 = conv_bn_relu(64 + channels[0], 64, kernel=3, stride=1, padding=1)
        self.gd_dec0 = conv_bn_relu(
            64 + 64,
            self.num_neighbors,
            kernel=3,
            stride=1,
        ...
        if self.args.conf_prop:
            ...
            # 1/1
            self.cf_dec1 = conv_bn_relu(
                64 + channels[0], 32, kernel=3, stride=1, padding=1
            )
            self.cf_dec0 = nn.Sequential(
                nn.Conv2d(
                    32 + 64, 1, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1)
                ),
                nn.Sigmoid(),
            )
