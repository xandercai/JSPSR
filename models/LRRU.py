...
from models.JSPSR import Model as jspsr
...
class Guide(nn.Module, ABC):
    def __init__(
        self, input_planes, weight_planes, norm_layer=None, weight_ks=1, input_ks=3
    ):
        super().__init__()
        if norm_layer is None:
            norm_layer = nn.BatchNorm2d
...
class BasicDepthEncoder(nn.Module, ABC):
    def __init__(self, kernel_size, block=BasicBlock, bc=16, norm_layer=nn.BatchNorm2d):
        ...
        self.conv_weight = nn.Conv2d(
            bc * 4, self.kernel_size**2, kernel_size=1, stride=1, padding=0
        )
        self.conv_offset = nn.Conv2d(
            bc * 4, 2 * (self.kernel_size**2 - 1), kernel_size=1, stride=1, padding=0
        )
...
class Post_process_deconv(nn.Module, ABC):
    def __init__(self, args):
        ...
        self.padding = ((args.kernel_size - 1) // 2, (args.kernel_size - 1) // 2)
        # self.dilation = 1
        self.dilation = (1, 1)
...
class Model(nn.Module, ABC):
    def __init__(
        ...
        self.in_channels = self.args.input_channels
        self.out_channels = self.args.output_channels
        assert len(self.in_channels) > 1, "At least 2 input data are required"
        ...
        self.layer1_img, self.layer1_lidar = self._make_layer(
            block, in_channels * 2, layers[0], stride=1
        )
        self.guide1 = guide(in_channels * 2, in_channels * 2, norm_layer, weight_ks)

        self.inplanes = in_channels * 2 * block.expansion
        self.layer2_img, self.layer2_lidar = self._make_layer(
            block, in_channels * 4, layers[1], stride=2
        )
        self.guide2 = guide(in_channels * 4, in_channels * 4, norm_layer, weight_ks)

        self.inplanes = in_channels * 4 * block.expansion
        self.layer3_img, self.layer3_lidar = self._make_layer(
            block, in_channels * 8, layers[2], stride=2
        )
        self.guide3 = guide(in_channels * 8, in_channels * 8, norm_layer, weight_ks)

        self.inplanes = in_channels * 8 * block.expansion
        self.layer4_img, self.layer4_lidar = self._make_layer(
            block, in_channels * 8, layers[3], stride=2
        )
        self.guide4 = guide(in_channels * 8, in_channels * 8, norm_layer, weight_ks)

        self.inplanes = in_channels * 8 * block.expansion
        self.layer5_img, self.layer5_lidar = self._make_layer(
            block, in_channels * 8, layers[4], stride=2
        )
        ...
        self.weight_offset0 = BasicDepthEncoder(
            kernel_size=3, block=BasicBlock, bc=bc, norm_layer=nn.BatchNorm2d
        )
        ...
        self.weight_offset1 = BasicDepthEncoder(
            kernel_size=3, block=BasicBlock, bc=bc, norm_layer=nn.BatchNorm2d
        )
    ...
    def forward(self, *in_tensor):
        depth, img, _, _, _ = jspsr.parse_input(True, False, False, False, *in_tensor)
        lidar = depth
        d_clear = depth
        ...
        #     bz = lidar.shape[0]
        #     self.dep_max = torch.max(lidar.view(bz, -1), 1, keepdim=False)[0].view(
        #         bz, 1, 1, 1
        #     )
    ...
    def _make_layer(self, block, planes, blocks, stride=1):
        ...
        for _ in range(1, blocks):
            m = torch.distributions.bernoulli.Bernoulli(torch.Tensor([self.prob_now]))
            img_layers.append(
                block(self.prob_now, m, self.multFlag, self.inplanes, planes)
            )
            depth_layers.append(
                block(self.prob_now, m, self.multFlag, self.inplanes, planes)
            )
